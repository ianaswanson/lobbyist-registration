...
export function AppealsClient() {
  ...
  const handleScheduleHearing = async (appealId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage({
          type: "error",
          text: error.error || "Failed to schedule hearing",
        });
      }
    } catch (error) {
  ...
  const handleDecideAppeal = async () => {
    if (!selectedAppeal || !decisionOutcome || !decisionText.trim()) {
      setMessage({
        type: "error",
        text: "Please provide both outcome and decision text",
      });
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        setMessage({
          type: "success",
          text: `Appeal ${decisionOutcome.toLowerCase()} successfully!`,
        });
        ...
        setMessage({
          type: "error",
          text: error.error || "Failed to decide appeal",
        });
      }
    } catch (error) {
      ...
      setMessage({
        type: "error",
        text: "Error deciding appeal. Please try again.",
      });
    } finally {
      setSubmitting(false);
    }
    ...
    <div className="container mx-auto max-w-7xl py-8">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          Review and decide on violation appeals per §3.809 (30-day appeal
          window)
        </p>
      </div>
      ...
      <div className="mb-8 grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-muted-foreground text-xs">Upcoming hearings</p>
          </CardContent>
        </Card>
                  ...
                  <TableBody>
                    {filterAppealsByStatus(tab).length === 0 ? (
                      ...
                      filterAppealsByStatus(tab).map((appeal) => {
                        const daysPending = getDaysSinceSubmission(
                          ...
                          >
                            ...
                            <TableCell>
                              <span className="font-semibold">
                                ${appeal.violation.fineAmount}
                              </span>
                            </TableCell>
                            <TableCell>
                              <Badge
                                className={appealStatusColors[appeal.status]}
                              >
                                {appeal.status}
                              </Badge>
                            </TableCell>
          ...
          {selectedAppeal && (
            <div className="space-y-4 py-4">
                ...
                <AlertDescription>
                  <div className="mt-2 space-y-1 text-sm">
                    <div>
                      ...
                      {
                        violationTypeLabels[
                          selectedAppeal.violation.violationType
                        ]
                      }
                    </div>
                    <div>
                      <strong>Fine:</strong> $
                      {selectedAppeal.violation.fineAmount}
                    </div>
                    <div>
                      <strong>Issued:</strong>{" "}
                      {new Date(
                        selectedAppeal.violation.issuedDate
                      ).toLocaleDateString()}
                    </div>
                    <div>
                      <strong>Description:</strong>{" "}
                      {selectedAppeal.violation.description}
                    </div>
                  </div>
                </AlertDescription>
              </Alert>
              ...
              <div>
                ...
                <div className="mt-2 rounded-md bg-gray-50 p-4 text-sm">
              ...
              <div className="grid grid-cols-3 gap-4 text-sm">
                <div>
                  ...
                  <p className="text-muted-foreground text-xs">
                    {getDaysSinceSubmission(selectedAppeal.submittedDate)} days
                    ago
                  </p>
                </div>
                <div>
                  ...
                  <p>
                    {new Date(
                      selectedAppeal.appealDeadline
                    ).toLocaleDateString()}
                  </p>
                  <p className="text-muted-foreground text-xs">
                    {getDaysUntilDeadline(selectedAppeal.appealDeadline)} days
                    remaining
                  </p>
                </div>
                <div>
      ...
      <Dialog open={decisionDialogOpen} onOpenChange={setDecisionDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Make a final decision on this appeal. This action updates the
              violation status.
            </DialogDescription>
          </DialogHeader>
          ...
          {selectedAppeal && (
            <div className="space-y-4 py-4">
              ...
              <div className="rounded-md bg-gray-50 p-3 text-sm">
                ...
                {violationTypeLabels[selectedAppeal.violation.violationType]} -
              ...
              <div className="space-y-2">
                ...
                />
                <p className="text-muted-foreground text-xs">
                  This will be communicated to the appellant and recorded in the
                  violation record.
                </p>
              </div>
            </div>
