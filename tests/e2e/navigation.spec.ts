...
test.describe("Navigation", () => {
  ...
  test("should display admin navigation menu", async ({ page }) => {
    // Check logo
    await expect(
      page.locator('nav a:has-text("Lobbyist Registry")').first()
    ).toBeVisible();

    // Check admin menu items (use first match to avoid strict mode violations)
    await expect(
      page.locator('nav a[href="/dashboard"]').first()
    ).toBeVisible();
    await expect(
      page.locator('nav a:has-text("Review")').first()
    ).toBeVisible();
    await expect(
      page.locator('nav a:has-text("Compliance")').first()
    ).toBeVisible();
    await expect(
      page.locator('nav a:has-text("Violations")').first()
    ).toBeVisible();
    await expect(
      page.locator('nav a:has-text("Search")').first()
    ).toBeVisible();

    // Exemption Checker should NOT be visible for admin in nav
    const navBar = page.locator("nav").first();
    await expect(
      navBar.locator('a:has-text("Exemption Checker")')
    ).not.toBeVisible();
  });
...
test.describe("Lobbyist Navigation", () => {
  ...
  test("should display lobbyist navigation menu", async ({ page }) => {
    ...
    await expect(
      page.locator('nav a:has-text("Register")').first()
    ).toBeVisible();
    await expect(
      page.locator('nav a:has-text("Hour Tracking")').first()
    ).toBeVisible();
    await expect(
      page.locator('nav a:has-text("My Reports")').first()
    ).toBeVisible();
    await expect(
      page.locator('nav a:has-text("Search")').first()
    ).toBeVisible();
    await expect(
      page.locator('nav a:has-text("Exemption Checker")').first()
    ).toBeVisible();
  });
  ...
  test("should highlight active hour tracking navigation item", async ({
    page,
  }) => {
    ...
    const hourTrackingLink = page
      .locator('nav a:has-text("Hour Tracking")')
      .first();
...
test.describe("Public Navigation", () => {
  test("should display public navigation on exemption checker", async ({
    page,
  }) => {
    ...
    await expect(
      nav.locator('a:has-text("Lobbyist Registry")').first()
    ).toBeVisible();
    await expect(
      nav.locator('a:has-text("Exemption Checker")').first()
    ).toBeVisible();
