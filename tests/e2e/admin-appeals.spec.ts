...
test.describe.skip(
  ...
  () => {
    ...
    test("should display appeals dashboard with summary cards", async ({
      ...
      await expect(
        page.locator("text=Hearings Scheduled").first()
      ).toBeVisible();
    ...
    test("should display pending appeals in table", async ({ page }) => {
      ...
      await expect(
        page.getByRole("columnheader", { name: /submitted/i })
      ).toBeVisible();
      await expect(
        page.getByRole("columnheader", { name: /violation/i })
      ).toBeVisible();
      await expect(
        page.getByRole("columnheader", { name: /fine amount/i })
      ).toBeVisible();
    });
    ...
    test("should open appeal details dialog", async ({ page }) => {
      ...
      const viewButton = page
        .locator('button[data-testid="view-appeal"], button:has(svg)')
        .first();
    ...
    test("should schedule a hearing for pending appeal", async ({ page }) => {
      ...
      if (await viewButton.isVisible()) {
        ...
        if (await hearingInput.isVisible()) {
          ...
          await expect(
            page.getByText(/hearing scheduled successfully/i)
    ...
    test("should decide an appeal - uphold fine", async ({ page }) => {
      ...
      if (await decideButton.isVisible()) {
        ...
        await textarea.fill(
          "After careful review of the evidence and the appellant's arguments, the violation is upheld. The appellant failed to provide sufficient justification for the delay in registration."
        );

        // Submit decision
        await page.click(
          'button:has-text("UPHELD Appeal"), button:has-text("Uphold")'
        );

        // Should show success message
        await expect(page.getByText(/appeal upheld successfully/i)).toBeVisible(
          {
            timeout: 10000,
          }
        );
    ...
    test("should decide an appeal - overturn fine", async ({ page }) => {
      ...
      if (await decideButton.isVisible()) {
        ...
        const textarea = page.locator('textarea[id="decision-text"]');
        await textarea.fill(
          "Upon review, the appellant has demonstrated that the delay was due to circumstances beyond their control. The violation is overturned and the fine is dismissed."
        );

        // Submit decision
        await page.click(
          'button:has-text("OVERTURNED Appeal"), button:has-text("Overturn")'
        );

        // Should show success message
        await expect(
          page.getByText(/appeal overturned successfully/i)
        ).toBeVisible({ timeout: 10000 });
      }
    });
    ...
    test("should validate decision text is required", async ({ page }) => {
      ...
      if (await decideButton.isVisible()) {
        ...
        const submitButton = page.locator(
          'button:has-text("UPHELD Appeal"), button:has-text("Decide Appeal")'
        );
        await expect(submitButton).toBeDisabled();
      }
    });

    test("should show error message for invalid hearing date", async ({
      page,
    }) => {
      // Navigate to PENDING tab
      await page.click('button[role="tab"]:has-text("Pending")');
      ...
      if (await viewButton.isVisible()) {
        ...
        if (await scheduleButton.isVisible()) {
          ...
          await expect(
            page.getByText(/please select a hearing date/i)
          ).toBeVisible({ timeout: 5000 });
        }
      }
    });

    test("should highlight urgent appeals (>20 days pending)", async ({
      page,
    }) => {
    ...
    test("should navigate between tabs without losing state", async ({
      page,
    }) => {
