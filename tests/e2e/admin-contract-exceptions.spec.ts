...
test.describe('Admin Contract Exceptions Management', () => {
  ...
  test('should display contract exceptions page with ordinance information', async ({ page }) => {
    // Check page header
  ...
  test('should create a new contract exception', async ({ page }) => {
    ...
    await page.fill('textarea[id="contract-desc"]', 'Professional services contract for IT consulting related to cybersecurity infrastructure upgrade.')

    await page.fill('textarea[id="justification"]', 'This exception is warranted because: (1) The contract is in the best interests of the County as Mr. Smith has unique expertise in our legacy systems, and (2) His influence on contract authorization was minimal as he served in an advisory capacity only. The cost savings from avoiding a full RFP process and training a new consultant are estimated at $50,000.')
    ...
    await expect(page.getByText(/exception created successfully/i)).toBeVisible({ timeout: 10000 })
  })

  test('should validate required fields when creating exception', async ({ page }) => {
    await page.click('button:has-text("Create Exception")')
    await page.waitForSelector('role=dialog')
  ...
  test('should edit an existing exception', async ({ page }) => {
    ...
    const editButton = page.locator('button:has(svg)').filter({ hasText: '' }).first()
    ...
    if (await editButton.isVisible()) {
      ...
      await justificationField.fill(currentValue + ' [Updated with additional details]')

      // Submit
      await page.click('button:has-text("Update Exception")')

      // Should show success message
      await expect(page.getByText(/exception updated successfully/i)).toBeVisible({ timeout: 10000 })
    }
  })
  ...
  test('should delete an exception with confirmation', async ({ page }) => {
    ...
    const deleteButton = page.locator('button:has(svg)').filter({ hasText: '' }).last()
    ...
    if (await deleteButton.isVisible()) {
      ...
      await expect(page.getByText(/exception deleted successfully/i)).toBeVisible({ timeout: 10000 })
    }
  })
  ...
  test('should cancel delete when user declines confirmation', async ({ page }) => {
    ...
    if (await deleteButton.isVisible()) {
      ...
      await expect(page.getByText(/exception deleted successfully/i)).not.toBeVisible()
    }
  })
  ...
  test('should display posted vs not posted badges correctly', async ({ page }) => {
  ...
  test('should show legal requirements alert in create dialog', async ({ page }) => {
    await page.click('button:has-text("Create Exception")')
    await page.waitForSelector('role=dialog')
  ...
  test('should show success message after toggling post status', async ({ page }) => {
    const toggleButton = page.locator('button:has(svg)').filter({ hasText: '' }).first()
    ...
    if (await toggleButton.isVisible()) {
      ...
      const successAlert = page.locator('[class*="bg-green"]').filter({ hasText: /success/i })
