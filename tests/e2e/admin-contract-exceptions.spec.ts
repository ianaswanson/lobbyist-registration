...
test.describe.skip(
  ...
  () => {
    ...
    test("should display contract exceptions page with ordinance information", async ({
      ...
      // Check page header
      await expect(
        page.getByText("Contract Exception Management")
      ).toBeVisible();

      // Check ordinance alert
      await expect(
        page.getByText("Contract Regulation (§9.230)")
      ).toBeVisible();
    ...
    test("should create a new contract exception", async ({ page }) => {
      ...
      await expect(
        page.getByText(/exception created successfully/i)
    ...
    test("should view exception details", async ({ page }) => {
      ...
      if (await viewButton.isVisible()) {
        ...
        await expect(
          page.getByText("Contract Exception Details")
        ).toBeVisible();
    ...
    test("should prevent editing official ID in edit mode", async ({
      page,
    }) => {
      const editButton = page
        .locator("button:has(svg)")
