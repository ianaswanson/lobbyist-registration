...
// TODO: Fix element visibility and form interaction issues in user workflows
// These tests are skipped due to:
// 1. Elements not becoming visible (toBeVisible failures)
// 2. Form validation issues
// 3. Hour tracking activity history problems
// 4. Registration requirement info not displaying
// See Issue #42 for tracking
test.describe.skip(
  "User Workflows (SKIPPED - needs visibility/form fixes)",
  () => {
    ...
    test.describe("Expense Reporting Flow", () => {
      ...
      test("should show multiple input modes", async ({ page }) => {
        ...
        await expect(
          page.locator('button:has-text("Manual Entry")')
        ).toBeVisible();
        await expect(
          page.locator('button:has-text("CSV Upload")')
        ).toBeVisible();
        await expect(
          page.locator('button:has-text("Paste Data")')
        ).toBeVisible();
      });
    ...
    test.describe("Public Exemption Checker Flow", () => {
      ...
      test("should display exemption questions", async ({ page }) => {
        ...
        await expect(
          page.locator(
            "text=How many hours per quarter do you spend on lobbying activities?"
          )
        ).toBeVisible();
        ...
        await expect(
          page.locator("text=I am a government official")
        ).toBeVisible();
        await expect(
          page.locator("text=I only give public testimony")
        ).toBeVisible();
      });
    ...
    test.describe("Hour Tracking Flow", () => {
      ...
      test("should display quarterly summary card", async ({ page }) => {
        ...
        await expect(
          page.locator("text=hours until registration required")
        ).toBeVisible();
      });
      ...
      test("should display hour log form", async ({ page }) => {
        ...
        // Check form fields
        await expect(
          page.locator('label:has-text("Activity Date")')
        ).toBeVisible();
        await expect(page.locator('label:has-text("Hours")')).toBeVisible();
        await expect(
          page.locator('label:has-text("Activity Description")')
        ).toBeVisible();
        await expect(
          page.locator('button:has-text("Add Hours")')
        ).toBeVisible();
      });
      ...
      test("should add a new hour log entry", async ({ page }) => {
        ...
        await page.fill(
          "textarea",
          "Meeting with county commissioners about transportation policy"
        );
        ...
        await expect(
          page.locator("text=Meeting with county commissioners")
        ).toBeVisible();
      ...
      test("should show registration requirement info", async ({ page }) => {
        ...
        await expect(
          page.locator("text=Registration Requirement")
        ).toBeVisible();
      ...
      test("should update progress bar color as hours increase", async ({
        page,
      }) => {
        await page.goto("/hours");
    ...
    test.describe("Public Search Flow", () => {
      ...
      test("should show advanced filters", async ({ page }) => {
        ...
        await expect(
          page.locator('label:has-text("Min Expense Amount")')
        ).toBeVisible();
      });
