...
test.describe('Admin Violations Management', () => {
  ...
  test('should display violations dashboard with summary cards', async ({ page }) => {
  ...
  test('should display violations table with tabs', async ({ page }) => {
    // Check for tabs
    await expect(page.getByRole('tab', { name: /all violations/i })).toBeVisible()
    await expect(page.getByRole('tab', { name: /active fines/i })).toBeVisible()
    await expect(page.getByRole('tab', { name: /under appeal/i })).toBeVisible()
  ...
  test('should issue a violation with fine', async ({ page }) => {
    ...
    // Fill in description
    await page.fill('textarea[id="description"]', 'Lobbyist failed to register within 3 working days after exceeding 10 hours threshold.')
    ...
    await expect(page.getByText(/violation issued successfully/i)).toBeVisible({ timeout: 10000 })
  })

  test('should issue a warning without fine (educational letter)', async ({ page }) => {
    await page.click('button:has-text("Issue Violation")')
    await page.waitForSelector('role=dialog')
    ...
    await page.fill('textarea[id="description"]', 'First-time late report submission. Issuing educational letter.')
  ...
  test('should validate required fields when issuing violation', async ({ page }) => {
    await page.click('button:has-text("Issue Violation")')
    await page.waitForSelector('role=dialog')

    // Try to submit without filling fields
    const submitButton = page.locator('button:has-text("Issue Violation")').last()
  ...
  test('should view violation details', async ({ page }) => {
    ...
    const viewButton = page.locator('button:has(svg)').filter({ hasText: '' }).first()
  ...
  test('should display violation type badges correctly', async ({ page }) => {
  ...
  test('should show appeal information for violations under appeal', async ({ page }) => {
  ...
  test('should display educational letter indicator for first-time violations', async ({ page }) => {
  ...
  test('should show success message after issuing violation', async ({ page }) => {
    await page.click('button:has-text("Issue Violation")')
    await page.waitForSelector('role=dialog')
    ...
    await page.fill('textarea[id="description"]', 'Employer failed to submit quarterly expense report by deadline.')
  ...
  test('should navigate back to dashboard from violations page', async ({ page }) => {
