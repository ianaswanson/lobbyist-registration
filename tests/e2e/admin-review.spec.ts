...
// TODO: Fix element visibility and button interaction issues in admin review pages
// These tests are skipped due to:
// 1. Buttons not becoming visible (toBeVisible failures)
// 2. Elements not found - needs data-testid attributes for reliable selection
// 3. Brittle text-based selectors breaking (e.g., button:has-text("Approve"))
// 4. Processing state and success message timing issues
// See Issue #42 for tracking
test.describe.skip(
  "Admin Review (SKIPPED - needs visibility/element fixes)",
  () => {
    test.describe("Admin Review - Lobbyist Registrations", () => {
      ...
      test("should display pending registrations", async ({ page }) => {
        ...
        await expect(
          page.locator("text=Community Advocacy Group")
        ).toBeVisible();
      ...
      test("should approve a registration", async ({ page }) => {
        ...
        await expect(
          page.locator("text=No Pending Registrations")
        ).toBeVisible();
      });
    ...
    test.describe("Admin Review - Expense Reports", () => {
      ...
      test("should show both lobbyist and employer report types", async ({
        ...
        await expect(
          page.locator("text=Lobbyist Expense Report")
        ).toBeVisible();

        // Should have employer report badge
        await expect(
          page.locator("text=Employer Expense Report")
        ).toBeVisible();
      });
...
); // End of Admin Review skip wrapper
