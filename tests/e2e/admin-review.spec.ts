...
test.describe('Admin Review - Lobbyist Registrations', () => {
  ...
  test('should display pending registrations', async ({ page }) => {
    // Check page title
    await expect(page.locator('h1')).toContainText('Review Lobbyist Registrations')

    // Should see at least one pending registration
    const registrationCards = page.locator('[class*="rounded-lg border bg-white"]')
  ...
  test('should approve a registration', async ({ page }) => {
    ...
    const firstCard = page.locator('[class*="rounded-lg border bg-white"]').first()
    ...
    await expect(page.locator('text=Registration approved successfully')).toBeVisible({ timeout: 5000 })

    // Registration card should disappear from list
    await expect(page.locator('text=Michael Chen')).not.toBeVisible({ timeout: 5000 })
  ...
  test('should reject a registration with reason', async ({ page }) => {
    ...
    await page.fill('textarea[placeholder*="reason"]', 'Missing required authorization documentation')
    ...
    // Wait for success message
    await expect(page.locator('text=Registration rejected')).toBeVisible({ timeout: 5000 })
...
test.describe('Admin Review - Expense Reports', () => {
  ...
  test('should show both lobbyist and employer report types', async ({ page }) => {
  ...
  test('should approve an expense report', async ({ page }) => {
    ...
    // Wait for success message
    await expect(page.locator('text=Expense report approved successfully')).toBeVisible({ timeout: 5000 })
  ...
  test('should reject an expense report with notes', async ({ page }) => {
    ...
    await page.fill('textarea[placeholder*="reason"]', 'Expense receipts do not match reported amounts')
    ...
    // Wait for success message
    await expect(page.locator('text=Expense report rejected')).toBeVisible({ timeout: 5000 })
  })

  test('should request clarification on an expense report', async ({ page }) => {
    // Find the clarification button
    const firstCard = page.locator('[class*="rounded-lg border bg-white"]').first()
    const clarifyButton = firstCard.locator('button:has-text("Request Clarification")')

    // Click request clarification
    await clarifyButton.click()

    // Should show textarea for clarification message
    await expect(page.locator('textarea[placeholder*="clarification"]')).toBeVisible()
    ...
    // Wait for success message
    await expect(page.locator('text=Clarification requested')).toBeVisible({ timeout: 5000 })
  })

  test('should require notes when rejecting or requesting clarification', async ({ page }) => {
    ...
    const submitRejectButton = page.locator('button:has-text("Submit Rejection")')
    ...
    // Submit button should be disabled without notes
    const submitClarifyButton = page.locator('button:has-text("Submit Request")')
...
test.describe('Admin Review - Navigation', () => {
  ...
  test('should navigate between registration and report review pages', async ({ page }) => {
