...
function parseCalendarCSV(text: string): CSVParseResult<CalendarEntry> {
  ...
  dataLines.forEach((line, index) => {
    ...
    if (parts.length < 4) {
      parseErrors.push(
        `Row ${index + 2}: Expected 4 columns (Event Title, Date, Time, Participants), got ${parts.length}`
      );
      return;
    }
    ...
    if (!datePattern.test(parts[1].trim())) {
      parseErrors.push(
        `Row ${index + 2}: Invalid date format "${parts[1].trim()}". Expected YYYY-MM-DD`
      );
      return;
    }
    ...
    if (!timePattern.test(parts[2].trim())) {
      parseErrors.push(
        `Row ${index + 2}: Invalid time format "${parts[2].trim()}". Expected HH:MM`
      );
      return;
    }
